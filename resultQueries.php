...
if(!(isset($_POST["action"]) && count($filteredPreIDs) == 0)){
	...
	//order task results by testID and order in test
	// foreach ($results as $key => $row) {
	// 	$testIDSorted[$key]  = $row['testID'];
	// 	$orderInTestSorted[$key] = $row['orderInTest'];
	// }
	// array_multisort($testIDSorted, SORT_ASC, $orderInTestSorted, SORT_ASC, $results);}
